<template>
    <div class="m-nowshowing">
        <div class="m-item clearfix" v-for="item in movieList" :key="item.id">   
            <div class="pic-show"><img :src="item.img | setWH('130.180')" alt=""></div>
            <div class="info_list">
                <h2 class="clearfix">
                    <em>{{item.nm}}</em>
                    <img src="/img/imaxs.png" v-if="item.version!=''"/>
                </h2>
...
.m-nowshowing .info_list p .grade{
    color: #fea54c;
    font-size: 14px;
    margin-left: 6px;
}
.m-nowshowing .btn_mall{
